...
$sel_product = "SELECT product.pid,product.category,product.product_name,product.brand,product.mrp,product.selling_price,product.qty,product.images,product.title,product.description,product.ingredients,product.howto,cart.uid,cart.pid,cart.qty from product  inner join cart on product.pid = cart.pid where cart.uid = '".$uid."' ";
	 	...
	 	<table>
	 		...
	 		<?php while($row = mysqli_fetch_array($res1)) { ?>
	 				<tr class="cart_item" >
	 					...
	 					<td>
	 						 <a href="product_detail.php?pid=<?php echo $row['pid'];?>"><img src="image\product\<?php echo $row['images'];?>" class="image" width="100" height="100"> </a>
	 					</td>
	 					<td> 
	 						<p> <?php echo $row['product_name'];?> </p>
	 						<p style="color: gray;"> Brand : <?php echo $row['brand'];?> </p>
	 					</td>
	 					<td>
	 						<form action="manage_cart.php" method="POST"> 
	 						 	<input type="number" name="change_qty" class="pqty" value="<?php echo $row['qty']?>" min="1" max="5" onchange="this.form.submit();"> 
	 						 	<input type="hidden" name="pid" value="<?php echo $row['pid'];?>">
	 						</form>
	 					</td>
	 					<td> <?php echo $row['selling_price'];?> <input type="hidden" class="pprice" value="<?php echo $row['selling_price'];?>"> </td>
	 					<td class="ptotal">  </td>
	 					<td>
	 						
	 					
	 						    <a href="cart_delete_product.php?pid=<?php echo $row['pid'];?>"><button type="submit" class="remove" name="remove" > Remove </button> </a>
